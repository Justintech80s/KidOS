...
function Resolve-RunnerAsset {
    ...
    if (-not $asset.digest -or $asset.digest -notmatch '^sha256:[0-9a-fA-F]{64}$') {
        throw 'The official GitHub Actions runner release asset did not provide a valid SHA-256 digest.'
    }
...
function Install-RunnerFiles {
    ...
    try {
        ...
        $expectedSha256 = ($asset.digest -split ':', 2)[1].ToLowerInvariant()
        $actualSha256 = (Get-FileHash -LiteralPath $archive -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($actualSha256 -ne $expectedSha256) {
            throw "GitHub Actions runner archive SHA-256 verification failed. Expected $expectedSha256 but downloaded $actualSha256."
        }
