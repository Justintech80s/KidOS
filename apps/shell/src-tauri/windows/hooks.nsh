...
!macro NSIS_HOOK_POSTINSTALL
  ...
  ; Generate a private classifier token and protect it under ProgramData.
  nsExec::ExecToStack '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "$d=$env:ProgramData+''\KidOS\Guardian''; New-Item -ItemType Directory -Force -Path $d | Out-Null; $b=New-Object byte[] 32; [System.Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($b); $t=([Convert]::ToHexString($b)).ToLowerInvariant(); Set-Content -Path ($d+''\media-classifier.token'') -Value $t -NoNewline -Encoding ASCII; icacls $d /inheritance:r /grant:r ''SYSTEM:(OI)(CI)(F)'' ''Administrators:(OI)(CI)(F)'' | Out-Null"'
  Pop $0
  Pop $1
  ${If} $0 != 0
    MessageBox MB_ICONSTOP|MB_OK "KidOS could not provision the private media-classifier credential. Installation will stop."
    Abort
  ${EndIf}
