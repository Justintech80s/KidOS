...
export default function KidOSTopBar({ onSafeSearch }: { onSafeSearch(query: string): void }) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); const value = query.trim(); if (value) onSafeSearch(value); }
  return (
    <header className="kidos-topbar">
      <form className="kidos-global-search" onSubmit={submit}>
        <span aria-hidden="true">⌕</span><input aria-label="Search KidOS safely" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the safe web..." /><button type="submit">Search</button>
      </form>
