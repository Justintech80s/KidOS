...
assert.match(bootstrap, /\.digest/, 'runner release asset must provide a cryptographic digest');
assert.match(bootstrap, /Get-FileHash[\s\S]*SHA256/, 'downloaded runner archive must be verified with SHA-256 before extraction');
assert.match(bootstrap, /runner archive SHA-256/i, 'bootstrap must fail closed when runner archive integrity verification fails');
